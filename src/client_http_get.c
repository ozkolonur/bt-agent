...
int http_get_init(client_t *client)
{
	printf("init http-get client\n");
	printf("Using mining platform : %s\n", HTTP_GET_MINING_PLATFORM);
	return 0;
}

#define RES_COMMITTED 0
int http_get_send(char *email, char *password, kvlist_t **result)
{
	CURL *curl;
	CURLcode res;
...
int http_get_halt(client_t *client)
{
	return 0;
}
